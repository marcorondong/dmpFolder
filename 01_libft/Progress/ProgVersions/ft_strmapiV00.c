...
/*   Updated: 2023/09/11 20:46:46 by marondon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strmapi(char const *s, char (*f)(unsigned int, char))
{
	...
	if (!s) // Validate if s exist
		return (NULL);
	i = ft_strlen(s);
	newstr = (char *)malloc((i + 1) * sizeof(char)); // Allocate space for null-terminator
	if (!newstr)
		return (NULL);
	i = 0;
	while (s[i] != '\0')
	{
		newstr[i] = (*f)(i, s[i]); // Or: f(i, s[i])
		i++;
	}
