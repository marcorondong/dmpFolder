...
/*   Updated: 2023/09/12 15:50:23 by marondon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_free_arr(char **arr, unsigned int word) // BEFORE: word was not a pointer (*)
{
	while (word > 0)
	{
		free(arr[word]);
...
static char	**ft_fillarr(char const *s, char c, char **arr, unsigned int *word) // BEFORE: static char	**ft_fillarr(char const *s, char c, char **arr)
{
	size_t			i;
	unsigned int	charcount;
	//unsigned int	word; // TESTING
	...
	while (s[i] != '\0' && arr)
	{
		if (s[i] != c)
		{
			...
			if (s[i + 1] == c || s[i + 1] == '\0')
			{
				arr[*word] = ft_substr(s, (i - charcount + 1), charcount); // BEFORE:arr[word] = .....
				if (!arr[*word]) // BEFORE: if (!arr[word])
