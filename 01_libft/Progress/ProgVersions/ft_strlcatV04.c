...
size_t	ft_strlcat(char *dst, const char *src, size_t size)
{
	...
	i = 0;
	while (dst[i] != '\0' && i < size) // in full string (strlen(dst)+1 = size), when i = size, then s[i] == '\0'
		i++;
	j = 0;
	while (src[j] != '\0' && i + j + 1 < size) // This check if there's enough space to fit (dst + src + '\0')
	{
		dst[i + j] = src[j];
		j++;
	}
	if (i < size) // This checks if strlen(dst) < size. if not, then dst will not be NULL terminated
		dst[i + j] = '\0';
	while (src[j] != '\0')
		j++;
	return (i + j);
...
- strlcat return the size of the string they tried to create. So it always
