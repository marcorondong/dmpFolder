...
/*   Updated: 2023/09/11 13:33:26 by marondon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	**ft_fillarr(char const *s, char c, char **arr)
{
	...
	while (s[i] != '\0')
	{
		...
		/*
		charcount++; // This must be here because Im reseting it inside the IF
		if ((s[i] == c && (s[i - 1] != c && i != 0)) || s[i + 1] == '\0') //DOUBT: i > 0. (s[i] != c && (s[i + 1] = c || s[i + 1] = '\0'))
		{
			arr[word] = ft_substr(s, (i - charcount + 1), charcount - 1); // Check.
			word++;
			charcount = 0;
		}
		i++;
...
/* ====== 1st try: print all but the last char in string
static char	**ft_fillarr(char const *s, char c, char **arr)
{
	size_t			i;
	unsigned int	charcount;
