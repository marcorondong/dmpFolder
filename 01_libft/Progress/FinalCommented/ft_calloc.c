...
/*   Updated: 2023/09/14 14:14:42 by marondon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*ft_calloc(size_t nmemb, size_t size)
{
	...
	// MAN: If nmemb or size is 0, then calloc() returns NULL, or a unique
	...
	// Is equal to ptr = malloc(nmemb * size); since malloc returns void*
	// Malloc protection:
	if (ptr == NULL)
		return (NULL);
	// Call bzero to initialize everything to '\0'
...
pointer value that can later be successfully passed to free().
-> So: if (size == 0 || nmemb == 0) Then return (malloc(0));
If the multiplication of nmemb and size would result in  integer  overflow,
then calloc() returns an error.
-> So: if (((long int)size * nmemb) > 2147483647) Then return (NULL);
If I check ((size * nmemb) / nmemb != size) I could have division by zero
So I shouldn't check like that.
- void allows to use any data type; and 'size_t' is 'unsigned long int' type
