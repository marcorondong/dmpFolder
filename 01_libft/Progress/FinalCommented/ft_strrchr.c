...
char	*ft_strrchr(const char *s, int c)
{
	...
	i = ft_strlen(s); // Use len of 's' to traverse it backwards
	while (i > 0)
	{
		// I'm comparing char so I have to cast c from 'int' to 'char'
		// and also i want to compare the actual char. Since char goes
		// from -128 to 127, but int is more.
		if (s[i] == (char)c)
		{
			// Since s is pointer (address) I can add 'i' to move
			// to the address of the 1st occurence and return it
			// as 'char *'
			return ((char *)(s + i));
		}
		i--;
	}
	// Must add this check since size_t is unsinged int, and it'll never
	// be less than zero '0'. so here I check when i=0.
	if (s[i] == (char)c)
		return ((char *)(s + i));
	return (NULL);
}
