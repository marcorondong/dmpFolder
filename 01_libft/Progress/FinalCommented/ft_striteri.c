...
/*   Updated: 2023/09/12 10:55:04 by marondon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_striteri(char *s, void (*f)(unsigned int, char*))
{
	...
	{
		// '(*f)' is the same as 'f'
		(*f)(i, &s[i]); // Use '&' because (*f) needs pointer (address)
		// I'm changing the REAL value of s[i], so I don't need to
		// asing it again to s[i]
		i++;
	}
	return ;
}
...
==============================================================================
- (*f) is equal to f
- I shouldn't do this: s[i] = (*f)(i, &s[i]); Because I'm passing s[i]
directly (with &s[i]). So it's not necessary to assign again s[i]
*/
