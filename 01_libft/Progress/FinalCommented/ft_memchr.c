...
void	*ft_memchr(const void *s, int c, size_t n)
{
	...
	while (i < n)
	{
		// Cast 'const void *s' to 'unsigned char *'. To treat memory
		// as an array of bytes (unsigned chars) rather than an array
		// of some other data type. (char = 1 byte)
		if (((unsigned char *)s)[i] == (unsigned char)c)
		{
		// '&' to send the address (pointer) of memory's i-th byte
		// Cast '*s' to 'void*' to match return data type
